...
ghreply ngethostbyname(unsigned char *host,unsigned char *dns_server, int type, int print_messages) {
    ...
    qinfo->qtype = htons( type ); // sending query of A record type
    ...
    for(i=0; i<ntohs(dns->ans_count); i++)
    {
        ...
        {
            answers[i].preference = htons(*(uint16_t*)reader);
            reader += 2;
        ...
        else if(ntohs(answers[i].resource->type)== T_SOA)
        {
            ...
            answers[i].rname = ReadName(reader,buf,&stop);
            reader = reader + stop;

            answers[i].serial = htonl(*(uint32_t*)reader);
            reader += 4;

            answers[i].refresh = htonl(*(uint32_t*)reader);
            reader += 4;

            answers[i].retry = htonl(*(uint32_t*)reader);
            reader += 4;

            answers[i].expire = htonl(*(uint32_t*)reader);
            reader += 4;
        }
        else
        {
    ...
    for(i=0 ; i < ntohs(dns->ans_count) ; i++)
    {
        ...
        if( ntohs(answers[i].resource->type) == T_A) //IPv4 address
        {
            ...
            if(print_messages)
                printf("A %s",inet_ntoa(a.sin_addr));
            if(retval.type == -2) {
                retval.type = 0;
        ...
        if(ntohs(answers[i].resource->type) == T_CNAME ||
                ntohs(answers[i].resource->type) == T_NS ||
                ntohs(answers[i].resource->type) == T_MX)
        {
            ...
            case T_NS:
                ...
                break;
            case T_MX:
                printf("MX %d", answers[i].preference);
                break;
            }
            //Simple name
            if(print_messages)
        ...
        {
            printf("SOA %s %s %u %u %u %u", answers[i].rdata, answers[i].rname,
                   answers[i].serial, answers[i].refresh, answers[i].retry, answers[i].expire);
        }
